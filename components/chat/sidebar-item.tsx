...
export function SidebarItem({ index, chat, children }: SidebarItemProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="max-h-5 w-3/4 flex-1 select-none overflow-hidden text-ellipsis break-all"
          title={chat.title}
        >
          {/* <span className="whitespace-nowrap">
            {shouldAnimate ? (
              ...
              <span className="turnicate w-1/2">{chat.title}</span>
            )}
          </span> */}
          <div className="w-3/4">{chat.title}</div>
        </div>
      </Link>
